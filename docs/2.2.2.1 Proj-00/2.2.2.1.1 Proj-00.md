...
噢差点忘了！还有[补注](./2.2.2.1.2%20Notes.md)可以看喵！建议配合着一起食用！
...
随后进入`Customize Installation`自定义安装。

> 你也不想安装程序在你C盘里的一个偏僻角落拉史罢（）

如果没有什么特别的需要，无脑跟着图片选也不失为一种速通方式，但是最好还是结合一下自己的实际情况（）

![Install Step 1](https://cdn.xyxsw.site/install_step_1.png)
...
6. 为所有用户安装
   > 这个选项会影响到添加到的环境变量是全局的还是当前用户的，以及不同用户的py启动器的内容也是隔离开的。需要你根据实际情况做决策。
